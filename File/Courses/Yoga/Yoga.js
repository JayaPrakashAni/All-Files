...
const localCourseImage = require('../../Assets/Instruments/Yoga.jpg'); 
...
const Yoga = () => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: 'Yoga Courses' });
  }, [navigation]);
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/app/getcourses/11`);
