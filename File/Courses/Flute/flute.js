...
// Import a local image to replace the API image
const BASE_URL = 'https://my.bmusician.com';
const localImage = require('../../Assets/Instruments/Flute.jpg');
...
const Flute = () => {
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/app/getcourses/9`);
  ...
  const renderDescription = (description) => {
    ...
    const sentences = description
      ...
      .replace(/\s+/g, ' ')
      .trim()
      .split('.') // Split description into sentences
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {otherCourses.map((course) => (
        <View key={course.ID} style={styles.cardContainer}>
          {/* Replace API image with a local image */}
          <Image
            source={localImage}
