...
const localCourseImage = require('../../Assets/Instruments/ukulele.png'); 
...
const Ukulele = () => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: 'Ukulele Courses' });
  }, [navigation]);
  ...
  const getCourseFee = (courseName) => {
    ...
    if (name.includes('basics')) return '₹ 2000';
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/app/getcourses/32`);
