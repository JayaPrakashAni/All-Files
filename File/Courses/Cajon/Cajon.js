...
const localCourseImage = require('../../Assets/Instruments/Cajon.jpg'); // Update the path based on your local structure
...
const Cajon = () => {
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/app/getcourses/13`);
  ...
  const renderDescription = (description) => {
    ...
    return sentences.map((sentence, index) => (
      <Text key={index} style={styles.bulletText}>
        {sentence.trim()} {/* Displaying text without bullet points */}
      </Text>
    ));
  };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {otherCourses.map((course) => (
        <View key={course.ID} style={styles.cardContainer}>
          ...
          <Image
            source={localCourseImage} // Use local image here
