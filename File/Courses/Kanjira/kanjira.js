...
const localImage = require('../../Assets/Instruments/Kanjira.jpg'); // Replace with the correct path to your local image
...
const Kanjira = () => {
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await fetch(`${BASE_URL}/app/getcourses/15`);
  ...
  const renderDescription = (description) => {
    ...
    return points.slice(0, 3).map((point, index) => (
      <Text key={index} style={styles.descriptionText}>
        {point.trim()} {/* Display as plain text */}
      </Text>
    ));
  };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {otherCourses.map((course) => (
        <View key={course.ID} style={styles.cardContainer}>
          {/* Replaced remote image with local image */}
          <Image
