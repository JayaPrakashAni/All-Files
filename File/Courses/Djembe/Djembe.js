...
// Import local image
const localImage = require('../../Assets/Instruments/Djembe.png');
...
const Djembe = () => {
  ...
  useEffect(() => {
    const fetchOtherCourses = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/app/getcourses/12`);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {otherCourses.map((course) => (
        <View key={course.ID} style={styles.cardContainer}>
          {/* Use the same local image for all courses */}
          <Image source={localImage} style={styles.courseImage} />
